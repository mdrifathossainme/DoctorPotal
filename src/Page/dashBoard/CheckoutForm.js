 ...
 useEffect(()=>{
    ...
    if (!stripe || !elements) {
      ...
      const {error, paymentMethod} = await stripe.createPaymentMethod({
       ...
       const {paymentIntent, error:intenError} = await stripe.confirmCardPayment(
        clientSecret,
        {
          payment_method: {
            card: card,
    ...
    return (
      ...
      />
      <button className='btn btn-success btn-sm mt-2' type="submit" disabled={!stripe||!clientSecret}>
        Pay
      </button>
         </form>
         {
